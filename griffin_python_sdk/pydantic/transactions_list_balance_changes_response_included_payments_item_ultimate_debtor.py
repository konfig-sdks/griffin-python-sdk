...
class TransactionsListBalanceChangesResponseIncludedPaymentsItemUltimateDebtor(BaseModel):
    # Must be a UK BBAN.
    account-number_: str = Field(alias='account-number')

    account-number-code_: Literal["bban"] = Field(alias='account-number-code')

    # Must be a UK Sort Code.
    bank-id_: str = Field(alias='bank-id')

    bank-id-code_: Literal["gbdsc"] = Field(alias='bank-id-code')

    # The name of the [account holder](http://docs.griffin.com).
    account-holder_: typing.Optional[str] = Field(None, alias='account-holder')
