...
class PayeesListLegalPersonPayeesResponsePayeesItem(BaseModel):
    # Link to the [payee](http://docs.griffin.com).
    payee-url_: str = Field(alias='payee-url')

    # Must be a UK BBAN.
    account-number_: str = Field(alias='account-number')

    # The name of the [account holder](http://docs.griffin.com).
    account-holder_: str = Field(alias='account-holder')
    ...
    bank-id_: str = Field(alias='bank-id')

    # ISO 8601 formatted date-time.
    created-at_: datetime = Field(alias='created-at')

    # The status of the payee; you can only make payments to active payees.
    payee-status_: Literal["deactivated", "active"] = Field(alias='payee-status')

    # ISO 3166-1 alpha-2 two-letter country code.
    country-code_: typing.Union[bool, date, datetime, dict, float, int, list, str, None] = Field(alias='country-code')

    # Link to the bank account resource.
    account-url_: typing.Optional[str] = Field(None, alias='account-url')
    class Config:
