...
class ApiKeysCreateKeyResponse(BaseModel):
    # The resource path for the API key
    api-key-url_: str = Field(alias='api-key-url')

    # The immutable display name for the API key
    api-key-name_: typing.Union[bool, date, datetime, dict, float, int, list, str, None] = Field(alias='api-key-name')

    api-key-live?_: bool = Field(alias='api-key-live?')
    ...
    # The API secret
    api-key-secret_: str = Field(alias='api-key-secret')
    class Config:
