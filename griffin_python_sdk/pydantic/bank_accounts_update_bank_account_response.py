...
from griffin_python_sdk.pydantic.bank_accounts_update_bank_account_response_account_balance import BankAccountsUpdateBankAccountResponseAccountBalance
from griffin_python_sdk.pydantic.bank_accounts_update_bank_account_response_available_balance import BankAccountsUpdateBankAccountResponseAvailableBalance
from griffin_python_sdk.pydantic.bank_accounts_update_bank_account_response_bank_addresses import BankAccountsUpdateBankAccountResponseBankAddresses

class BankAccountsUpdateBankAccountResponse(BaseModel):
    # Link to the [payment submissions](http://docs.griffin.com) debiting from this account.
    account-submissions-url_: str = Field(alias='account-submissions-url')

    # Specifies whether the bank account has restrictions applied by Griffin.
    account-restricted_: bool = Field(alias='account-restricted')

    # Link to the [payments](http://docs.griffin.com) associated with this account.
    account-payments-url_: str = Field(alias='account-payments-url')

    # Link to the [payment admissions](http://docs.griffin.com) crediting to this account.
    account-admissions-url_: str = Field(alias='account-admissions-url')
    ...
    bank-product-type_: Literal["savings-account", "client-money-account", "safeguarding-account", "operational-account"] = Field(alias='bank-product-type')

    # The mutable display name for the bank account
    display-name_: str = Field(alias='display-name')

    # Link to the [legal person](http://docs.griffin.com) that represents the [controller](http://docs.griffin.com) of the account.
    controller-url_: str = Field(alias='controller-url')

    # Specifies whether the bank account holds funds belonging to multiple beneficiaries.
    pooled-funds_: bool = Field(alias='pooled-funds')
    ...
    account-status_: Literal["closing", "open", "closed", "opening"] = Field(alias='account-status')

    # Link to the [legal person](http://docs.griffin.com) that represents the [owner](http://docs.griffin.com) of the account.
    owner-url_: str = Field(alias='owner-url')

    # ISO 8601 formatted date-time.
    created-at_: datetime = Field(alias='created-at')

    # Link to the endpoint that enables account closure for this account.
    close-account-url_: str = Field(alias='close-account-url')

    available-balance_: BankAccountsUpdateBankAccountResponseAvailableBalance = Field(alias='available-balance')

    # Link to the [transactions](http://docs.griffin.com) associated with this account.
    account-transactions-url_: str = Field(alias='account-transactions-url')

    # Link to the bank account resource.
    account-url_: str = Field(alias='account-url')

    account-balance_: BankAccountsUpdateBankAccountResponseAccountBalance = Field(alias='account-balance')

    # Link to the list of [pool members](http://docs.griffin.com) associated with this account.
    pooled-account-memberships-url_: typing.Optional[str] = Field(None, alias='pooled-account-memberships-url')

    # Specifies the type of client money account.
    client-money-type_: typing.Optional[Literal["designated-client-fund", "designated-client-money", "general-client-money"]] = Field(None, alias='client-money-type')

    # Link to manage [pooled account membership](http://docs.griffin.com).
    pooled-account-membership-updates-url_: typing.Optional[str] = Field(None, alias='pooled-account-membership-updates-url')

    bank-addresses_: typing.Optional[BankAccountsUpdateBankAccountResponseBankAddresses] = Field(None, alias='bank-addresses')

    # Link to the [legal person](http://docs.griffin.com) that represents the [beneficiary](http://docs.griffin.com) of the account.
    beneficiary-url_: typing.Optional[str] = Field(None, alias='beneficiary-url')

    # Specifies the type of savings account.
    savings-type_: typing.Optional[Literal["easy-access"]] = Field(None, alias='savings-type')
    class Config:
