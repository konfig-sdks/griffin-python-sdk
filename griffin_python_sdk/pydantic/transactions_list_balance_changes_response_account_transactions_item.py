...
from griffin_python_sdk.pydantic.transactions_list_balance_changes_response_account_transactions_item_account_balance import TransactionsListBalanceChangesResponseAccountTransactionsItemAccountBalance
from griffin_python_sdk.pydantic.transactions_list_balance_changes_response_account_transactions_item_balance_change import TransactionsListBalanceChangesResponseAccountTransactionsItemBalanceChange

class TransactionsListBalanceChangesResponseAccountTransactionsItem(BaseModel):
    # Link to the transaction resource.
    account-transaction-url_: str = Field(alias='account-transaction-url')

    # When the transaction was posted.
    post-datetime_: datetime = Field(alias='post-datetime')

    # Specifies the direction of a balance change.
    balance-change-direction_: Literal["credit", "debit"] = Field(alias='balance-change-direction')

    # Specifies the origin of a balance change, for example a payment
    transaction-origin-type_: Literal["interest", "deposit", "fee", "transfer", "payment", "commission"] = Field(alias='transaction-origin-type')

    # Link to the bank account resource.
    account-url_: str = Field(alias='account-url')

    balance-change_: TransactionsListBalanceChangesResponseAccountTransactionsItemBalanceChange = Field(alias='balance-change')

    account-balance_: TransactionsListBalanceChangesResponseAccountTransactionsItemAccountBalance = Field(alias='account-balance')

    # Link to the payment resource.
    payment-url_: typing.Optional[str] = Field(None, alias='payment-url')
