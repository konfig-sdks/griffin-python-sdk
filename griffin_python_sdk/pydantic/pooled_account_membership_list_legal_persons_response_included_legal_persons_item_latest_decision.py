...
class PooledAccountMembershipListLegalPersonsResponseIncludedLegalPersonsItemLatestDecision(BaseModel):
    # A link to the [verification](http://docs.griffin.com).
    verification-url_: str = Field(alias='verification-url')

    decision-outcome_: Literal["accepted", "declined"] = Field(alias='decision-outcome')

    # Indicates if the decision was automated or made by a human.
    decision-maker_: Literal["system", "ops-user", "user"] = Field(alias='decision-maker')

    # Free-text field to explain the reasons behind the decision.
    decision-notes_: str = Field(alias='decision-notes')
    ...
    decision-user-url_: typing.Optional[str] = Field(None, alias='decision-user-url')

    # Username of the [user](http://docs.griffin.com) that made the decision (if applicable).
    decision-user-username_: typing.Optional[str] = Field(None, alias='decision-user-username')

    # Indicates that an Ops user has created the decision.
    decision-ops-user_: typing.Optional[Literal["griffin-ops-user"]] = Field(None, alias='decision-ops-user')
    class Config:
